...
void motoare_init_pini(void) {
    ...
    PORTB |= (1 << EN_PIN); // Dezactivat (Liniște totală)
}
...
void step_motor(uint8_t motor_idx, uint16_t pulses, uint8_t dir) {
    PORTB &= ~(1 << EN_PIN); // Activare punți drivere
    _delay_ms(5);
    ...
    _delay_ms(5);
    PORTB |= (1 << EN_PIN); // Silent-mode în repaus
}
...
void executa_mutare_string(const char* mutare) {
    ...
    if (mutare[1] == '\0') {
        step_motor(motor, 50, 1);  
    } else if (mutare[1] == '\'') {
        step_motor(motor, 50, 0);  
    } else if (mutare[1] == '2') {
        step_motor(motor, 100, 1); 
    }
}
...
void executa_lista_miscari(char* lista) {
    ...
    while (token != NULL) {
        ...
        _delay_ms(400); // Stabilizare mecanică
        token = strtok(NULL, " ");
    }
}